...
layui.use(['form','table','layer'],function () {
    ...
    pageData.getTableData = function(searchKey) {
        ...
        table.on('tool(roleTableEvent)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'add'){
                ...
                var update_url = common.url.page_root + common.url.model.account.page.update + '?id='+obj.data.id;
    ...
    pageData.openAddModel = function(){
        layer.open({
            title: common.optName.CONTROLLER_OPT_ADD + common.url.model.account.name,
            type:1,//页面类型
            content:$('#add_form'),
            area:['600px'],
            btn:['提交'],
            yes: function(index, layero){//当前层索引、当前层DOM对象
                ...
                var bankName = $(layero).find("input[name=bankName]").val();
                var webUserName = $(layero).find("input[name=webUserName]").val();
                var webPassword = $(layero).find("input[name=webPassword]").val();
                ...
                seg = $.trim(seg);
                remark = $.trim(remark);
                if(!name || !userName || !bankName || !webUserName || !webPassword || !seg || !remark){
    ...
    //添加提交数据
    pageData.submitAdd = function(account){
        common.sendOption.data = account;
    ...
    pageData.openUpdateModel = function(modelObj){
        ...
        layer.open({
            title:common.optName.CONTROLLER_OPT_UPDATE + common.url.model.account.name,
            type:1,//页面类型
            content:$('#add_form'),
            area:['600px'],
            btn:['提交'],
    ...
    pageData.submitUpdateRole = function(id,name,seg,remark){
        ...
        common.sendOption.url = common.url.web_root + common.url.model.role.action + common.url.opt.update;
        common.sendOption.type = common.sendMethod.POST;
    ...
    pageData.deleteSubmit = function(obj){
        ...
        common.sendOption.url = common.url.web_root + common.url.model.account.action + common.url.opt.delete;
    ...
    $(document.body).on('click','#addBtn',function () {
        // pageData.openAddModel();
        var add_url = common.url.page_root + common.url.model.account.page.add;
        window.open(add_url);
    });
