...
layui.use(['form','table','layer'],function () {
    ...
    var tableHeader = [[ //表头
        ...
        ,{field: 'open',            title: '排序',      align:'center'}
        ,{field: 'createUserId',    title: '说明',        align:'center'}
        ,{fixed: 'right', width:150, align:'center', toolbar: '#toolbarRight'} //这里的toolbar值是模板元素的选择器
    ]];

    role.getTableData = function() {
        //执行渲染
        table.render({
            ...
            , parseData: function (res) { //res 即为原始返回的数据  为 layui 2.4.0 开始新增
                return {
                    ...
                    "count": res.totalCount?res.totalCount:role.totalCount,// res.total, //解析数据长度
                    "data": JSON.parse(res.data2json) // res.data.item //解析数据列表
                };
            }
        ...
        //监听工具条
        table.on('tool(auditListTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
